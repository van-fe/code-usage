...
如果 Cursor 没有返回消费上限，可以在 CodeUsage 中设置“显示预算”。这个值只用于计算进度条和剩余百分比，不会修改 Cursor 后台的消费上限，也不会阻止实际消费。设置后，状态栏优先显示按量付费的参考剩余比例；未设置时显示套餐总用量的剩余比例。
...
3. 启动 CodeUsage，状态栏会出现仪表盘图标和已启用工具的剩余比例。
