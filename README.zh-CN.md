...
- 可主动开启 iCloud，同步经过裁剪的用量展示快照，供后续 iPhone 与小组件读取。
...
应用不包含遥测，不会上传用量数据到 CodeUsage 自己的服务器。iCloud 同步默认关闭；用户主动开启后，只把工具名称、套餐名称、用量指标、百分比、金额/数量和刷新时间写入用户自己的 CloudKit 私有数据库。访问令牌、刷新令牌、Cookie、本机数据库、文件路径、原始命令输出和服务端原始响应均不会进入同步模型。Cursor 与 Kiro 的部分客户端协议不是公开稳定 API；字段变化时，应用会尽量保留其它可用指标并显示明确错误。
